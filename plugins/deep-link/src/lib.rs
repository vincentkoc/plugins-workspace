...
//! opened with.
//!
//! On Windows, Linux and FreeBSD, protocol schemes can additionally be registered and unregistered
//! at runtime with [`DeepLink::register`] and [`DeepLink::unregister`]. On macOS, Android and iOS
...
mod imp {
    ...
    impl<R: Runtime> DeepLink<R> {
        ...
        /// ## Platform-specific:
        ///
        /// - **Linux / FreeBSD**: Can only unregister the scheme if it was initially registered with [`register`](`Self::register`). Refreshes the desktop database with `update-desktop-database`; if the command cannot run, a warning is logged and [`is_registered`](`Self::is_registered`) may keep returning `true`. May not work on older distros.
        /// - **macOS / Android / iOS**: Unsupported, will return [`Error::UnsupportedPlatform`](`crate::Error::UnsupportedPlatform`).
